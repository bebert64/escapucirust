...
use crate::{
    store::{
        actions,
        house::{open_door_mart1, set_current_room},
...
pub(crate) fn html() -> Html {
    ...
    let svg =
        yew::Html::from_html_unchecked(yew::AttrValue::from(include_str!("svgs/door_mart1.svg")));
        ...
        use_effect_with_deps(
            {
                ...
                move |code_displayed: &UseStateHandle<[i32; 7]>| {
                    if current_code == "TNEPRES" {
                        state.dispatch(actions![set_current_room(RoomMart1), open_door_mart1(),]);
                    } else {
                        for i in 0..7 {
